...
const StudentUploadPage: CustomNextPage = () => {
  ...
  const [learningObjectivesValue, setLearningObjectivesValue] = useState<
    string[]
  >([]);
    ...
    if (file) {
      try {
        ...
        if (learningObjectives && learningObjectives.length > 0) {
          learningObjectivesValue = learningObjectives.map(
            (outcome: { text: string }) => outcome.text
          );
        }
        ...
        if (ocrCheck.isSelectable) {
          console.log(
            "filebytes inside the if(ocrCheck.isSelectable)",
            fileBytes
          );
  ...
  const handleSubmit = async () => {
    ...
    if (selectedFile) {
      try {
        ...
        const docRef = await addDoc(collection(db, "SyllabiURL"), {
          fileUrl: storageRefPath,
        });
    ...
    <div>
      ...
      <Grid container>
        ...
        <Grid item xs={12} mt={3}>
          ...
          <HoverButton
            instructions="Submit a new transfer request here. 
        ...
        <Grid item xs={12}>
          <CustomTextField
            id="bl-institution"
            placeholder="Hotel transylvannia State"
            fullWidth
        ...
        <Grid item xs={12} display="flex" alignItems="center">
          <CustomFormLabel htmlFor="credits">Credits</CustomFormLabel>
        </Grid>
        <Grid item xs={12}>
          <CustomTextField
            id="bl-credit"
        ...
        {/* 5 */}
        <Grid item xs={12} display="flex" alignItems="center">
          <CustomFormLabel htmlFor="bl-grade">Grade</CustomFormLabel>
        </Grid>
        <Grid item xs={12}>
          <CustomTextField
            id="bl-grade"
        ...
        <Grid item xs={12}>
          <CustomTextField
            ...
            onChange={(event: { target: { value: SetStateAction<string> } }) =>
              setCourseCategoryValue(event.target.value)
            }
          />
        </Grid>
        {/* 7 */}
        <Grid item xs={12} display="flex" alignItems="center">
          <CustomFormLabel htmlFor="bl-type">Term Type</CustomFormLabel>
        </Grid>
        <Grid item xs={12}>
