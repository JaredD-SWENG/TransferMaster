...
async function callCSVFunc(fullText: string, usingResume: boolean) {
  ...
  try {
    const response = await axios.post(
      "https://64dctbfpypozkldozv3yvcdr740jwqyh.lambda-url.us-east-1.on.aws/",
      {
        FullText: fullText,
...
const CourseMatch: CustomNextPage = () => {
  ...
  useEffect(() => {
    checkFormCompletion();
  }, [courseNameValue, creditsValue, courseCategoryValue, selectedFile]);
  ...
  return (
    <>
      ...
      <Grid item xs={12} mt={3} mb={3}>
        <HoverButton
          instructions="This is the Course Matcher
...
export default withRole({
  Component: CourseMatch,
  roles: ["Student", "Transfer Specialist", "Faculty"],
});
