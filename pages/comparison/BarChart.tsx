...
const BarChart: React.FC<BarChartProps> = ({ syllabusComponents }: any) => {
  ...
  const medium = "#6f41b6";
  const high = "#56bdc6";
  const [mainClickedBarValue, setMainClickedBarValue] = useState<number | null>(
    null
  );
  const [loClickedBarValue, setLoClickedBarValue] = useState<number | null>(
    null
  );
  ...
  if (
    ...
    learningObjectives.score !== undefined &&
    textbook !== undefined &&
    textbook.score !== undefined &&
    gradingScheme !== undefined &&
    gradingScheme.score !== undefined
  ) {
