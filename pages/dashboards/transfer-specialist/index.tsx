        ...
        querySnapshot.forEach(async (doc) => {
          ...
          if (requestData.Reviewer) {
            try {
              ...
              reviewerData = reviewerSnapshot.exists()
                ? reviewerSnapshot.data()?.Name || ""
                : "";
            } catch (error) {
  ...
  const handleAssign =
    ...
    async (event: React.ChangeEvent<{ value: unknown }>) => {
      ...
      const querySnapshot = await getDocs(
        query(usersCollection, where("Name", "==", reviewerName))
      );
        ...
        if (reviewerDocId) {
          ...
          try {
            await updateDoc(requestRef, {
              Reviewer: reviewerDocId ? doc(db, "Users", reviewerDocId) : null,
            });
          ...
          const unsubscribe = onSnapshot(
            ...
            (querySnapshot) => {
              ...
              querySnapshot.forEach(async (doc) => {
                ...
                // Fetch the ExternalSyllabus document
                const syllabusSnapshot = await getDoc(
                  requestData.ExternalSyllabus
                );
                const syllabusData = syllabusSnapshot.data()?.CourseName;
                const syllabusCategory =
                  syllabusSnapshot.data()?.CourseCategory;
  ...
  return (
    <>
      <Grid item xs={12} mt={3} mb={3}>
        ...
        <HoverButton
          instructions="This is the dashboard for Transfer Credit Specialists.
              ...
              {requests.map((request) => (
                <TableRow key={request.id}>
                  ...
                  <TableCell>
                    <Button
                      variant="outlined"
                      onClick={() => viewMoreDetails(request.id)}
                    >
                      View Details
                    </Button>
                  </TableCell>
...
export default withRole({
  Component: TransferSpecialistDashboard,
  roles: ["Transfer Specialist"],
});
